...
//--------------------------------------------------------------------------------------------------------------------
	...
	//public void myAccountHasBeenCredited(String account, @Transform(MoneyConverter.class) Money creditAmount) throws Throwable {
        System.out.println("################################ <Step - @GIVEN> - Account should be there: " + accountHelper.getMyAccount());
		accountHelper.getMyAccount().credit(deposit);
		System.out.println("################################ <Step - @GIVEN> - Account and amount to add has been written to Msg Queue - wait for it to be processed. " + accountHelper.getMyAccount());
...
//	public void theBalanceOfMyAccountShouldBe(String fromAccount, Money accountBalance) {
    ...
    public void theBalanceOfMyAccountShouldBe(String fromAccount, Money accountBalance) throws InterruptedException {
		...
		while (!accountHelper.getMyAccount().getAccountBalance().equals(accountBalance)
				&& timeoutMilliSecs > 0){
			Thread.sleep(pollIntervalMilliSecs);
			timeoutMilliSecs -= pollIntervalMilliSecs;
			System.out.println("[@THEN] > 2. While() > Get Account Balance: " + accountHelper.getMyAccount().getAccountBalance() +
					" and Compare to account balance (parm): " + accountBalance);
		}

		System.out.println("[@THEN] > 3. Get Account Balance: " + accountHelper.getMyAccount().getAccountBalance() +
				" and Compare to account balance (parm): " + accountBalance);

		//int actualAmount = helper.getMyAccount().getAccountBalance().getDollars();
		Money actualAccountBalance = accountHelper.getMyAccount().getAccountBalance();
